...
This is a frontend-only static Vite app. After building, deploy the contents of `dist/` to any static hosting provider.
...
- GitHub Pages
- Cloudflare Pages
- Static hosting behind Nginx or Apache
...
If deploying from Windows PowerShell locally, use:
...
If you see an execution policy error like `npm.ps1 cannot be loaded`, run commands through `npm.cmd`:
...
The project uses remote Unsplash image URLs. Check network access and verify that image URLs in `src/data/restaurant.ts` and `src/data/gallery.ts` are still valid.

### WhatsApp does not open correctly

Check the `whatsapp` value in:

```text
src/data/restaurant.ts
```

The code strips non-digits before creating the `wa.me` link, so include a full international number.

## Documentation

The repository also includes project-specific design notes:

- `PRODUCT.md` describes product goals, users, brand personality, and accessibility expectations.
- `DESIGN.md` describes the visual system, design principles, component direction, and brand rules.

## License

No license has been specified yet. Add one before publishing this as an open-source project.
